...
module.exports = function(Model) {
  ...
  Model.byOwnerOfLatest = function(cb) {
    ...
    const sql = 'select * from wbnews originalNews INNER JOIN (select max(wbdate) as latestDate, owner from wbnews group by owner) anotherNews on originalNews.owner = anotherNews.owner and originalNews.wbdate = anotherNews.latestDate';
  ...
  Model.remoteMethod(
        'byOwnerOfLatest',
    {
      ...
      accepts: [],
      returns: {arg: 'data', type: ['wbfirm'], root: true},
    }
    );
};
